...
#include <inttypes.h> // Include for PRI format macros
...
void rx_task(void *arg)
{
    ...
    for(;;) {
        ...
        if(xQueueReceive(status->queue, (void * )&event, (TickType_t)portMAX_DELAY)) {
            // Incoming data ??
...
void motionPerformance(void) {
    if (verbose) {
        printf("[%4d] %4d -> %8" PRId32 " (%4d) <%d>   [%4d] %4d -> %8" PRId32 " (%4d) <%d>\n",
       leftSpeed,  master.mmps, (int32_t)master.mm, master.power, master.controlMode,
       rightSpeed, slave.mmps, (int32_t)slave.mm, slave.power, slave.controlMode);

    } else {
        printf ("(%4d mm/s %" PRId32 " mm) (%4d mm/s %" PRId32 " mm)\n",
                master.mmps, (int32_t)master.mm, slave.mmps, (int32_t)slave.mm);
    }
}
...
void displayBuffer(DRIVE_STATUS * status) {
    ...
    case SSPE:
        printf ("%s Velocity:  %" PRId16 " mm/s\n", status->name, bytesToInt16(status->RXbuf));
        break;

    case SPOS:
        printf ("%s Position:  %" PRId32 " mm\n", status->name, (int32_t)(bytesToInt32(status->RXbuf)));
        break;

    case SVOL:
        printf ("%s Voltage:   %" PRIu16 " mV\n", status->name, bytesToUInt16(status->RXbuf));
        break;

    case SAMP:
        printf ("%s Current:   %" PRIu16 " mA\n", status->name, bytesToUInt16(status->RXbuf));
        break;

    case SPOW:
        printf ("%s Power:     %" PRId16 " %%\n", status->name, bytesToInt16(status->RXbuf));
        break;
    ...
    case SDOG:
        printf ("%s WatchDog:  %" PRIu16 " mS\n", status->name, bytesToUInt16(status->RXbuf));
        break;

    case SMOT:
        printf ("%s Motion:  %" PRId16 " mm/s, %" PRId32 " mm, %" PRId16 " %%\n", status->name,
                bytesToInt16(status->RXbuf), (int32_t)(bytesToInt32(status->RXbuf+2)), bytesToInt16(status->RXbuf+6));
        break;
...
void displayStatus(DRIVE_STATUS * status) {
    printf("%s (%d) V %+5d P %+7ld W %+4d B %5d /%4d [%d]\n",
           status->name, status->pending,
           status->mmps, (long)status->mm, status->power,
           status->mV, status->mA, status->controlMode);
}
...
void saveResponse(DRIVE_STATUS * status) {
    ...
    if ((status->state & LOCAL_ESTOP) == LOCAL_ESTOP) {
        ...
        case SPOS:
            status->mm = (int32_t)(bytesToInt32(status->RXbuf));
            break;
        ...
        case SMOT:
            ...
            status->mm    = (int32_t)(bytesToInt32(status->RXbuf + 2));
